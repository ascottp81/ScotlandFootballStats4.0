...
@section('content')
    ...
    <div class="opponentsColumn">
        <div class="opponentsSectorHeading">
            <span class="opponentsSector">{{ $regions[0]->title }}</span>
            {!! $regions[0]->image !!}
            <p class="opponentsImageText">{{ $regions[0]->image_text }}</p>
        </div>
        @foreach ($britishOpponents as $opponent)
        <a href="/opponents/{{ $opponent->url }}" class="opponentLink titleBar"><span class="basicLineupsFlag" style="background-image: url('/img/flags/{{ $opponent->flag }}.gif');"></span>{{ $opponent->name }}</a>
        @endforeach
        
        <div class="opponentsSectorHeading">
            <span class="opponentsSector">{{ $regions[1]->title }}</span>
            {!! $regions[1]->image !!}
    ...
    <div class="opponentsColumn">
        <div class="opponentsSectorHeading">
            <span class="opponentsSector">{{ $regions[2]->title }}</span>
            {!! $regions[2]->image !!}
